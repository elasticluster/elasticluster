    ...
    def _build_inventory(self, cluster):
        ...
        for node in cluster.get_all_nodes():
            if node.type in self.groups:
                extra_vars = []
                # Add any group variable defined in the configuration
                # file as:
                # <nodetype>_var_<varname>=<value>
                if node.type in self.environment:
                    extra_vars = ['%s=%s' % (k, v) for k, v in \
                                  self.environment[node.type].items()]
                # Add any node variable, usually generated by the
                # cloud provider.
                if node.private_vars:
                    extra_vars.extend(['%s=%s' % (k, v) for k, v in \
                                       node.private_vars.items()])
